...
class Track():
    ...
    def compute_angles_from_features(self, fname_angles):
    
        # 
        fname_angles = np.load(fname_angles)
        print (fname_angles.shape)
        angles = np.zeros((fname_angles.shape[0], 
                            fname_angles.shape[1]))+np.nan
        dists = np.zeros((fname_angles.shape[0], 
                            fname_angles.shape[1]))+np.nan
        ...
        #
        for k in trange(fname_angles.shape[0]):
            for p in range(fname_angles.shape[1]):
                temp = fname_angles[k,p]
    ...
    def filter_tracks_spines(self):
        ...
        # 
        print ("selftracks spine; ", self.tracks_spine.shape)
        for k in self.animal_ids:
        ...
        if os.path.exists(fname_scores) == False:
            ...
            for n in trange(len(self.slp)):
                for a in range(len(self.slp[n])):
                    ...
                    idx = a
                    # if self.tracks_names.index(name) == 2:
                        # idx = self.tracks_names.index(name)-1
                    # else:
