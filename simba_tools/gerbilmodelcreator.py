...
class GerbilModelCreator(object):
    ...
    def create_model(self):
        ...
        # make training data using target samples and nontarget samples
        self.train_df = pd.concat([self.non_target_train_df, 
                                   self.target_train_df], axis=0).reset_index(drop=True).fillna(0)
    ...
    def test_model(self):
        ...
        # compute probabillies of model on test data
        self.y_proba = self.rf_clf.predict_proba(self.test_df.values)[:, 1]
        ...
        p, r, t = precision_recall_curve(self.test_y, self.y_proba)
