...
class fisheye():
    ...
    def CorrectPictures_dir(self, 
                            dir_root, 
                            destination_folder):
                                
                                
        img_names = np.sort(glob(dir_root+"*.jpg"))
        ...
        for fn in tqdm(img_names):
            ...
            orig = imread(fn)
            corrected = remap(orig, self.map_x, self.map_y, INTER_CUBIC)
            imwrite(join(destination_folder, split(fn)[-1]), corrected)  
    ...
    def LoadChessboardPicture(self, chessboard_filename, lcorner_filename, rcorner_filename):
        ...
        print (self.chessboard_image_gray.shape)
        print (self.chessboard_image.shape)
        print (self.lcorner_image.shape)
        print (self.rcorner_image.shape)
